    ...
    return (
      <div className="ambient grain flex min-h-[100dvh] items-center justify-center bg-cream">
        <div className="text-center">
          ...
          <p className="rise-in mt-6 font-display text-lg text-espresso-soft"><FormattedMessage id="report.scoring" /></p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="ambient grain flex min-h-[100dvh] items-center justify-center bg-cream">
        <main className="rise-in w-full max-w-md text-center">
          <p className="text-[10px] uppercase tracking-[0.2em] font-medium text-espresso-soft"><FormattedMessage id="report.noReportYet" /></p>
          <h1 className="mt-3 font-display text-2xl font-extrabold tracking-tight"><FormattedMessage id="report.notScored" /></h1>
  ...
  return (
    <div className="ambient grain min-h-[100dvh] bg-cream">
      <header className="mx-auto flex w-full max-w-4xl items-center justify-between px-4 pt-8 md:px-8">
        ...
        <span className="text-sm text-espresso-soft"><FormattedMessage id="report.header" values={{ title: session?.title ?? "" }} /></span>
      </header>
      ...
      <main className="mx-auto w-full max-w-4xl px-4 pb-24 pt-10 md:px-8">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rise-in rounded-card bg-espresso p-6 text-cream">
            <p className="text-[10px] uppercase tracking-[0.2em] text-cream/60"><FormattedMessage id="report.overall" /></p>
          ...
          <div className="rise-in rounded-card bg-paper p-6 ring-1 ring-hairline" style={{ "--rise-delay": "120ms" } as React.CSSProperties}>
            <p className="text-[10px] uppercase tracking-[0.2em] font-medium text-espresso-soft"><FormattedMessage id="report.coverage" /></p>
          ...
          <div className="rise-in rounded-card bg-persimmon-faint p-6" style={{ "--rise-delay": "240ms" } as React.CSSProperties}>
            <p className="text-[10px] uppercase tracking-[0.2em] font-medium text-persimmon-deep"><FormattedMessage id="report.session" /></p>
            <p className="mt-2 font-display text-xl font-bold">{session?.mode} · {session?.duration_min} min</p>
          </div>
        </div>

        {/* competencies */}
        <h2 className="mt-12 text-[10px] uppercase tracking-[0.2em] font-medium text-espresso-soft"><FormattedMessage id="report.competencies" /></h2>
                ...
                <ul className="mt-4 space-y-2">
                  {c.evidence.map((e, i) => (
                    <li key={i} className="flex flex-wrap items-baseline gap-2 text-sm">
                      ...
                      <span className="text-xs text-espresso-soft"><FormattedMessage id="report.turn" values={{ n: e.turn_seq }} /></span>
                    </li>
                  ))}
                </ul>
              </div>
