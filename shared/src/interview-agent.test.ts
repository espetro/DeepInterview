...
import { VOICE_TOOLS, buildPrompt, cutSentences, describeWhiteboardSnapshot } from "./interview-agent.ts";

describe("buildPrompt", () => {
  it("includes mode, plan and current question with hints", () => {
    const p = buildPrompt({ mode: "coach", plan: "plan text", currentQuestion: "q", hints: ["h1", "h2"] });
  ...
  it("lists documents when present", () => {
    const p = buildPrompt({ mode: "interview", documents: [{ name: "cv.pdf", text: "ten years" }] });
...
describe("VOICE_TOOLS", () => {
  it("has the three interview tools with JSON schema parameters", () => {
    expect(VOICE_TOOLS.map((t) => t.name)).toEqual(["update_question", "read_editor", "read_whiteboard"]);
...
describe("cutSentences", () => {
  ...
  it("handles CJK terminals with trailing quotes, merging short fragments", () => {
    ...
    expect(sentences).toEqual(["这是一个足够长的句子,用来测试中文分句。短.\"And this one is long enough!\""]);
    expect(rest).toBe("");
  });
});
