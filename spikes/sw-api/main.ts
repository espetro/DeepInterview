...
async function runRoundTrip(): Promise<void> {
  ...
  log(
    `POST /v1/sessions -> ${created.status} ${JSON.stringify(created.body)}`,
  ...
  for (const [speaker, text] of [
    ...
    log(
      `POST /v1/sessions/:id/turns (${speaker}) -> ${turn.status} seq=${turn.body.seq}`,
  ...
  log(
    `tools round-trip -> ${putTools.status}/${gotTools.status} editor=${JSON.stringify(gotTools.body.editor)}`,
...
async function checkPersistence(): Promise<void> {
  ...
  log(
    `GET /v1/sessions/${sid} after reload -> ${got.status} title=${JSON.stringify(got.body.title)}`,
    got.status === 200 ? "ok" : "bad",
  );
  const turns = await api(`/sessions/${sid}/turns`);
  log(
    `GET turns -> ${turns.status} count=${Array.isArray(turns.body) ? turns.body.length : "?"}`,
